...
def check_password(password):
    ...
    if bool(("!" in password) or ("@" in password) or ("#" in password) or ("?" in password)) == False:
        print("Please contains at least on special character from ! @ # ?")
        return False
    
    # check 3: contains letters both in uppercase and lowercase
    up_mix_low = any(l.isupper() for l in password) and any(l.islower() for l in password)
    if bool(up_mix_low) == False:
        print("Please include both lowercase and uppercase letters")
        return False
    
    # check 4: contains of both letters and numbers
    num_mix_lett = any(l.isdigit() for l in password)
    if bool(num_mix_lett) == False:
